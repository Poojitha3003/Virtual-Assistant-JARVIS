...
def add_madhu_contact(con, cursor):
    query = "INSERT INTO contacts VALUES (null,'madhu', '6305313849','null')"
    cursor.execute(query)
    con.commit()

def add_gopi_contact(con, cursor):
    query = "INSERT INTO contacts VALUES (null,'Gopi', '8919736998','null')"
    cursor.execute(query)
    con.commit()

# Add chinna's contact (replace with actual phone number)
cursor.execute("INSERT INTO contacts (name, mobile_no, email) VALUES (?, ?, ?)", ("chinna", "8341944224", None))
